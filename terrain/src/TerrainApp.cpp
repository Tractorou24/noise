...
namespace
{
    template <typename TRenderBackend> requires meta::implements<TRenderBackend, IRenderBackend>
    ...
    template <>
    const String FileExtensions<Backends::VulkanBackend>::SHADER = "spv";
    template <>
    const String FileExtensions<Backends::DirectX12Backend>::SHADER = "dxi";

    template <typename TRenderBackend> requires meta::implements<TRenderBackend, IRenderBackend>
    void init_render_graph(TRenderBackend* backend, SharedPtr<IInputAssembler>& input_assembler_state)
    {
        using RenderPass = typename TRenderBackend::render_pass_type;
        using RenderPipeline = typename TRenderBackend::render_pipeline_type;
        using PipelineLayout = typename TRenderBackend::pipeline_layout_type;
        using ShaderProgram = typename TRenderBackend::shader_program_type;
        using InputAssembler = typename TRenderBackend::input_assembler_type;
        using Rasterizer = typename TRenderBackend::rasterizer_type;
        using FrameBuffer = typename TRenderBackend::frame_buffer_type;
        ...
        input_assembler_state = std::static_pointer_cast<IInputAssembler>(inputAssembler);
        ...
        UniquePtr<RenderPass> renderPass = device->buildRenderPass("Opaque")
                                                 ...
                                                 .renderTarget("Depth/Stencil Target",
                                                               RenderTargetType::DepthStencil,
                                                               Format::D32_SFLOAT,
                                                               RenderTargetFlags::Clear,
                                                               {1.f, 0.f, 0.f, 0.f});

        // Map all render targets to the frame buffer.
        std::ranges::for_each(frameBuffers, [&renderPass](auto& frame_buffer) { frame_buffer->addImages(renderPass->renderTargets()); });
    ...
    {
        auto adapters = backend.listAdapters() | std::ranges::to<std::vector<const IGraphicsAdapter*>>();
        if (adapters.empty())
            throw std::runtime_error("Cannot find best graphics adapter, none found.");

        std::ranges::sort(adapters,
                          [](const IGraphicsAdapter* lhs, const IGraphicsAdapter* rhs)
                          {
                              if (lhs->type() != rhs->type())
                                  return lhs->type() < rhs->type();
                              return lhs->dedicatedMemory() < rhs->dedicatedMemory();
                          });
        return adapters.front()->uniqueId();
    }
}
...
namespace terrain
{
    ...
    void TerrainApp::initBuffers(IRenderBackend* backend)
    {
    ...
    void TerrainApp::updateCamera(const ICommandBuffer& commandBuffer, IBuffer& buffer) const
    {
    ...
    void TerrainApp::onInit()
    {
        ...
        auto startCallback = [this]<typename TBackend>(TBackend* backend)
        {
            ...
            auto adapter = backend->findAdapter(find_best_adapter(*backend));
            ...
            init_render_graph(backend, m_inputAssembler);
        ...
        this->onBackendStart<Backends::VulkanBackend>(startCallback);
        this->onBackendStop<Backends::VulkanBackend>(stopCallback);
        ...
        Backends::DirectX12ShaderProgram::suppressMissingRootSignatureWarning();

        // Register the DirectX 12 backend de-/initializer.
        this->onBackendStart<Backends::DirectX12Backend>(startCallback);
        this->onBackendStop<Backends::DirectX12Backend>(stopCallback);
    }

    void TerrainApp::onResize(const void* sender, ResizeEventArgs e)
    {
    ...
    void TerrainApp::keyDown(int key, int scancode, int action, int mods)
    {
        if (key == GLFW_KEY_F9 && action == GLFW_PRESS)
            this->startBackend<Backends::VulkanBackend>();

        if (key == GLFW_KEY_F10 && action == GLFW_PRESS)
            this->startBackend<Backends::DirectX12Backend>();
    ...
    void TerrainApp::updateWindowTitle()
    {
    ...
    void TerrainApp::handleEvents()
    {
        ::glfwPollEvents();
    }

    void TerrainApp::drawFrame()
    {
